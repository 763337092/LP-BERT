...
from train_fn import emb_infer_fn, wn18rr_infer_fn, finetuning_train_fn, finetuning_eval_fn
from eval_fn import get_triplet_candidate, safe_ranking, get_pred_topk
from loss_fn import FocalBCELoss
...
PRETRAINED_MODEL_PATH2 = f'{DATA_PATH}/{DATASET}/wn18rr_pretrain/'
CACHE_PATH = f'{DATA_PATH}/{DATASET}/wn18rr_finetune/'
...
def loss_fn(y_pred, y_true, weights):
    ...
    loss_fct = FocalBCELoss(alpha=0.8)
...
##### Get Test Candidates
test_score_dict, test_candicate_dict = wn18rr_infer_fn(model, device, BATCH_SIZE*2, dataset.test,
                                           dataset.subj2objs, dataset.obj2subjs, dataset.ent_list, ent_rel2emb, ent2emb, topk=RECALL_NUM)
print(f'Test Score: '
